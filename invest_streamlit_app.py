...
#import save_userstories as stf
...
def predict_invest_criteria(user_story, model, tokenizer, thresholds, device):
    #Replace "user" from user story to ACTOR for nois reduction with mask_actors
    #Preprocess Suffix for irrelevant words to reduce noise with clean_suffix_if_irrelevant
    #cleaned_input = clean_suffix_if_irrelevant(mask_actors(user_story))
    from preprocessing import clean_user_story
    cleaned_input = clean_user_story(user_story)



    # Prepare the input
    encoding = tokenizer(
        cleaned_input, #changed from user_story to user_input
        padding='max_length',
        truncation=True,
        max_length=256,
        return_tensors='pt'
...
def analyze_user_story(user_story, predictions):
    ...
    user_story_pattern = re.compile(r'As\s+(?:a|an)\s+(.+?),\s+I want\s+(.+?)\s+so that\s+(.+)', re.IGNORECASE)
...
def process_batch(user_stories, model, tokenizer, thresholds, device):
    ...
    for story in user_stories:
        ...
        #Saving batch user stories json file
        save_user_story_json(story)     
        # Predict and analyze
...
# Function to detect circular story
def is_circular_story(story):
    pattern = r"I want to ([^.,;]+) so that I can (\1|to \1)"
    return bool(re.search(pattern, story, re.IGNORECASE))\
    
# Function to save user stories to JSON file
def save_user_story_json(user_story):
    ...
    filename = f"user_stories_{today}.json"
    ...
    try:
        ...
        if os.path.exists(filename):
            try:
                ...
                logging.warning("Existing JSON file is corrupted. Creating a new one.")
                stories = []
        else:
            stories = []
...
def main():
    ...
    with tab1:
        st.markdown("****Evaluator can make mistake****")
            ...
            if st.button("Evaluate User Story", key="evaluate_single"):
                if user_story.strip():
                    with st.spinner("Analyzing your user story..."):
                        ...
                        #user_story = clean_suffix_if_irrelevant(mask_actors(user_story))
                        user_story = preprocess.clean_user_story(user_story)
                        st.markdown(f"clean US: {user_story}") 
                        # call user story save to json function
                        save_user_story_json(user_story)

                        #Saving solution for local and Huggingfaces Hub
                        #save_user_story_json(user_story=user_story, session_id=st.session_state["session_id"])
        ...
        if st.session_state.get('show_single_results', False):
            ...
            # Check if circular user story then show message and go further to show prediction. Otherwise show parts of the valid/invalid US
            if is_circular_story(user_story):
                st.warning("⚠️ This user story appears semantically circular and may not satisfy INVEST criteria.")
            else: 
